...
export default function AccountPage() {
  ...
  const fieldStyle: React.CSSProperties = {
    ...
    color: 'var(--color-foreground)', background: '#fff',
    outline: 'none', boxSizing: 'border-box',
    transition: 'border-color 0.2s ease',
  };
  const labelStyle: React.CSSProperties = {
    display: 'block', marginBottom: 8, fontSize: 12,
    fontFamily: 'var(--font-wordmark)', letterSpacing: '0.1em',
    textTransform: 'uppercase', color: 'var(--color-foreground-muted)',
  };
        ...
        <div style={{ maxWidth: 480, margin: '0 auto' }}>
          <RevealBlock>
            <span style={{ fontFamily: 'var(--font-wordmark)', fontSize: 11, letterSpacing: '0.2em',
              ...
              Your Account</span>
            <h1 style={{ margin: '0 0 32px', fontFamily: 'var(--font-heading)', fontWeight: 500,
              fontSize: 'clamp(2rem,4vw,2.8rem)', lineHeight: 1 }}>
              {tab === 'signin' ? 'Welcome back' : 'Create account'}</h1>
          </RevealBlock>
          ...
          {tab === 'signin' ? (
              ...
              <form onSubmit={e => { e.preventDefault(); setLoggedIn(true); }}
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <a href="#" style={{ fontSize: 13, color: 'var(--color-brand)', textDecoration: 'none' }}>Forgot?</a>
                  </div>
                ...
                onMouseLeave={e => (e.currentTarget as HTMLButtonElement).style.background='var(--color-brand)'}>Sign In</button>
                <p style={{ margin: 0, textAlign: 'center', fontSize: 13, color: 'var(--color-foreground-muted)' }}>
                  ...
                  <a href="#" onClick={e=>{e.preventDefault();setTab('register')}}
                    style={{ color: 'var(--color-foreground)', textDecoration: 'underline' }}>Create an account</a>
                </p>
              </form>
            </RevealBlock>
          ) : (
            <RevealBlock delay={0.15}>
              <form onSubmit={e => { e.preventDefault(); setLoggedIn(true); }}
                ...
                onMouseEnter={e => (e.currentTarget as HTMLButtonElement).style.background='var(--color-brand-dark)'}
                onMouseLeave={e => (e.currentTarget as HTMLButtonElement).style.background='var(--color-brand)'}>Create Account</button>
              </form>
            </RevealBlock>
          )}
        </div>
  ...
  return (
    <StorefrontShell>
      <div style={{ maxWidth: 'var(--page-width)', margin: '0 auto',
        padding: '48px clamp(20px,3vw,40px) 88px' }}>
        <RevealBlock>
          <span style={{ fontFamily: 'var(--font-wordmark)', fontSize: 11, letterSpacing: '0.2em',
            textTransform: 'uppercase', color: 'var(--color-brand)', display: 'block', marginBottom: 12 }}>
            Welcome back</span>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between',
            ...
            <button onClick={() => { setLoggedIn(false); setTab('signin'); }}
              style={{ background: 'transparent', border: 'none', cursor: 'pointer',
                fontSize: 13, color: 'var(--color-foreground-muted)', textDecoration: 'underline' }}>
              Sign out</button>
          </div>
        </RevealBlock>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(16px,3vw,32px)' }}>
          <RevealBlock delay={0.1} style={{ gridColumn: '1/-1' }}>
            <div style={{ background: 'var(--color-surface)', borderRadius: 'var(--rounded-block)', overflow: 'hidden' }}>
              <div style={{ padding: '24px 28px', borderBottom: '1px solid var(--color-border)',
                ...
                <h2 style={{ margin: 0, fontFamily: 'var(--font-heading)', fontWeight: 500, fontSize: 20 }}>
                  Order History</h2>
              ...
              {ORDERS.map(o => (
                <div key={o.id} style={{
                  ...
                  gap: '0 24px', alignItems: 'center',
                }}>
                  <span style={{ fontFamily: 'var(--font-wordmark)', fontSize: 11,
                    letterSpacing: '0.08em', color: 'var(--color-foreground-muted)' }}>{o.id}</span>
                  <div>
                  ...
                  <span style={{ fontSize: 12, padding: '5px 12px', borderRadius: 9999,
                    background: 'var(--color-igi-bg)', color: 'var(--color-igi)', fontWeight: 600 }}>
                    {o.status}</span>
          ...
          <RevealBlock delay={0.15}>
            <div style={{ background: 'var(--color-surface)', borderRadius: 'var(--rounded-block)', padding: '24px 28px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[['Name', firstName + ' Hassan'],['Email','laila@example.com'],['Phone','+20 100 000 0000'],['Location','Zamalek, Cairo']].map(([k,v]) => (
                  <div key={k} style={{ display: 'flex', justifyContent: 'space-between',
                    paddingBottom: 12, borderBottom: '1px solid var(--color-border)', gap: 16 }}>
              ...
              <button style={{ marginTop: 20, width: '100%', padding: '12px',
                borderRadius: 'var(--rounded-button)', border: '1.5px solid var(--color-border-dark)',
                background: 'transparent', fontFamily: 'var(--font-body)', fontSize: 14,
                cursor: 'pointer', color: 'var(--color-foreground)', transition: 'border-color 0.2s ease' }}
          ...
          <RevealBlock delay={0.2}>
            <div style={{ background: 'var(--color-brand)', borderRadius: 'var(--rounded-block)',
              padding: '28px', color: '#fff', display: 'flex', flexDirection: 'column', minHeight: 220 }}>
              <Shield size={28} style={{ marginBottom: 18, opacity: 0.7 }} />
              <h3 style={{ margin: '0 0 10px', fontFamily: 'var(--font-heading)', fontWeight: 500, fontSize: 20 }}>
                Personal Concierge</h3>
              <p style={{ margin: '0 0 auto', fontSize: 14, lineHeight: 1.65, opacity: 0.82 }}>
                Sizing, customization, or a private viewing — our team is here for you.</p>
              <a href={`https://wa.me/${WA_PHONE}`} target="_blank" rel="noreferrer"
                style={{ marginTop: 24, padding: '12px 22px', borderRadius: 'var(--rounded-button)',
                  background: '#fff', color: 'var(--color-brand)', border: 'none',
